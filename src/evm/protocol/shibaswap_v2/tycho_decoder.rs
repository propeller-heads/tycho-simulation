...
use super::{enums::FeeAmount, state::ShibaswapV2State};
use crate::{
...
impl TryFromWithBlock<ComponentWithState, BlockHeader> for ShibaswapV2State {
    type Error = InvalidSnapshotError;

    /// Decodes a `ComponentWithState` into a `ShibaswapV2State`. Errors with a `InvalidSnapshotError`
    ...
    async fn try_from_with_header(
        ...
        Ok(ShibaswapV2State::new(liquidity, sqrt_price, fee, tick, ticks))
    }
}
...
mod tests {
    ...
    fn shibaswap_v2_component() -> ProtocolComponent {
    ...
    fn shibaswap_v2_attributes() -> HashMap<String, Bytes> {
        vec![
    ...
    #[tokio::test]
    async fn test_shibaswap_v2_try_from() {
        let snapshot = ComponentWithState {
            state: ResponseProtocolState {
                component_id: "State1".to_owned(),
                attributes: shibaswap_v2_attributes(),
                balances: HashMap::new(),
            },
            component: shibaswap_v2_component(),
            component_tvl: None,
            entrypoints: Vec::new(),
        };

        let result = ShibaswapV2State::try_from_with_header(
            snapshot,
            header(),
            &HashMap::new(),
            &HashMap::new(),
        ...
        assert!(result.is_ok());
        let expected = ShibaswapV2State::new(
            100,
            U256::from(200),
            FeeAmount::Medium,
            300,
    ...
    async fn test_shibaswap_v2_try_from_invalid(#[case] missing_attribute: String) {
        // remove missing attribute
        let mut attributes = shibaswap_v2_attributes();
        ...
        let mut component = shibaswap_v2_component();
    ...
    #[tokio::test]
    async fn test_shibaswap_v2_try_from_invalid_fee() {
