...
impl ShibaswapV2State {
    /// Creates a new instance of `ShibaswapV2State`.
    ///
    /// # Arguments
    ...
    pub fn new(
        ...
        let spacing = ShibaswapV2State::get_spacing(fee);
        let tick_list = TickList::from(spacing, ticks);
        ShibaswapV2State { liquidity, sqrt_price, fee, tick, ticks: tick_list }
    }
    ...
    fn swap(
            ...
            && state.sqrt_price != price_limit
        {
            ...
            let (sqrt_price, amount_in, amount_out, fee_amount) = swap_math::compute_swap_step(
                state.sqrt_price,
                ShibaswapV2State::get_sqrt_ratio_target(sqrt_price_next, price_limit, zero_for_one),
                state.liquidity,
...
impl ProtocolSim for ShibaswapV2State {
    fn fee(&self) -> f64 {
    ...
    fn get_amount_out(
        ...
        trace!(?amount_in, ?token_a, ?token_b, ?zero_for_one, ?result, "SHIBASWAP V2 SWAP");
        let mut new_state = self.clone();
...
mod tests {
    ...
    fn test_delta_transition() {
        let mut pool = ShibaswapV2State::new(
            1000,
