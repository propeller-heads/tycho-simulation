...
/// Trait for Constant Function Market Maker (CFMM) protocols
pub trait CFMMProtocol {
...
impl<T: CFMMProtocol + Clone + 'static + std::fmt::Debug + Sync + Send>
TryFromWithBlock<ComponentWithState> for T
{
    type Error = InvalidSnapshotError;

    /// Decodes a `ComponentWithState` into a CFMM protocol state. Errors with a
    /// `InvalidSnapshotError` if either reserve0 or reserve1 attributes are missing.
    fn try_from_with_block(snapshot: ComponentWithState, _block: Header, _account_balances: &HashMap<Bytes, HashMap<Bytes, Bytes>>, _all_tokens: &HashMap<Bytes, Token>) -> impl Future<Output=Result<Self, Self::Error>> + Send + Sync
    where
        Self: Sized
    {
...
impl<T: CFMMProtocol + Clone + 'static + std::fmt::Debug + Sync + Send> ProtocolSim for T {
    fn fee(&self) -> f64 {
