...
use ekubo_sdk::{
    chain::evm::{Evm, EvmBasePool, EvmBasePoolKey, EvmPoolKey, EvmTokenAmount},
    math::tick::to_sqrt_ratio,
    quoting::{
        pools::base::{BasePoolConstructionError, BasePoolResources, BasePoolState},
...
impl PartialEq for BasePool {
    fn eq(&self, other: &Self) -> bool {
        self.key() == other.key() &&
            self.imp.ticks() == other.imp.ticks() &&
...
pub(super) fn impl_from_state(
    ...
    state: BasePoolState,
    ticks: Vec<Tick>,
) -> Result<EvmBasePool, BasePoolConstructionError> {
    EvmBasePool::new(key, state, ticks)
}
...
impl EkuboPool for BasePool {
    ...
    fn finish_transition(
        ...
        let new_initialized_ticks = (!changed_ticks.is_empty()).then(|| {
            let mut ticks = self.imp.ticks().to_vec();
        ...
        if let Some(ticks) = new_initialized_ticks {
            self.imp = impl_from_state(self.imp.key(), self.state, ticks).map_err(|err| {
