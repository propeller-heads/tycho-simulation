//! Archived history-based swap-to-price strategies
//!
//! These strategies were developed during benchmarking but are not the best performers.
//! They are kept here for reference and potential future use.
//!
//! The best-performing strategies are in `history_based.rs`:
//! - `BrentStrategy` (1% bracket threshold) - 3.9 mean iterations
//! - `BrentOriginalStrategy` (classical half-step) - 5.0 mean iterations
...
use crate::swap_to_price::{
    ProtocolSimExt, QuerySupplyResult, SwapToPriceError, SwapToPriceResult,
};

use super::history_based::{
    geometric_mean, run_search, safe_next_amount, HistoryPoint, IqiStrategy, SearchConfig,
    MIN_DIVISOR,
};
...
///
/// The `xi_threshold` parameter controls how aggressive the IQI acceptance is:
/// - 0.5 (default): Classic Chandrupatla criterion
/// - Lower values (0.3): More conservative, prefer bisection
/// - Higher values (0.7): More aggressive, prefer IQI more often
pub struct ChandrupatlaStrategy {
    /// Threshold for xi criterion. IQI is used if |xi| <= threshold
    /// and |1-xi| <= threshold. Default: 0.5
    pub xi_threshold: f64,
}

impl Default for ChandrupatlaStrategy {
    fn default() -> Self {
        Self { xi_threshold: 0.5 }
    }
}

impl ChandrupatlaStrategy {
    /// Create with custom xi threshold
    pub fn with_threshold(xi_threshold: f64) -> Self {
        Self { xi_threshold }
    }
}
...
impl ChandrupatlaStrategy {
    ...
    fn next_amount_with_threshold(
        ...
        if history.len() >= 3 {
            ...
            if Self::should_use_iqi(low_price, p2.price, high_price, target, xi_threshold) {
                if let Some(estimate) = IqiStrategy::iqi(
                    p1.amount_f64,
                    p1.price,
                    p2.amount_f64,
...
impl ProtocolSimExt for ChandrupatlaStrategy {
    fn swap_to_price(
        ...
        let threshold = self.xi_threshold;
        run_search(
            state,
            target_price,
            token_in,
            ...
            |history, low, low_price, high, high_price, target| {
                Self::next_amount_with_threshold(
                    history, low, low_price, high, high_price, target, threshold,
                )
            },
        )
    }
...
impl ItpStrategy {
    ...
    pub fn with_params(k1: f64, k2: f64, n0: u32) -> Self {
        Self { k1, k2, n0 }
    }

    fn next_amount_with_params(
        history: &[HistoryPoint],
        low: &BigUint,
        low_price: f64,
        high: &BigUint,
        ...
        let rho_bound =
            epsilon * 2.0_f64.powi((n_half + n0).saturating_sub(j) as i32) - bracket / 2.0;
...
impl ProtocolSimExt for ItpStrategy {
    fn swap_to_price(
        ...
        let (k1, k2, n0) = (self.k1, self.k2, self.n0);
        run_search(
            state,
            target_price,
            token_in,
            ...
            SearchConfig::swap_to_price(),
            |history, low, low_price, high, high_price, target| {
                Self::next_amount_with_params(
                    history, low, low_price, high, high_price, target, k1, k2, n0,
                )
            },
        )
    }
...
// =============================================================================
// Strategy: Ridders' Method
// =============================================================================

/// Ridders' method: exponential fitting for quadratic convergence
///
/// Ridders' method evaluates the function at the midpoint and uses an exponential
/// transformation to linearize the function, achieving quadratic convergence.
///
/// Formula: x₄ = x₃ + (x₃ - x₁) × sgn(f₁ - f₂) × f₃ / √(f₃² - f₁×f₂)
///
/// where x₃ = (x₁ + x₂)/2 is the midpoint.
///
/// This method has no tunable parameters - it's a specific formula.
/// However, we wrap it in a struct to allow future extensions if needed.
///
/// Reference: Ridders, C.F.J. (1979). "A New Algorithm for Computing a Single Root
/// of a Real Continuous Function". IEEE Trans. Circuits Systems 26, 979-980.
pub struct RiddersStrategy;

impl RiddersStrategy {
    fn next_amount(
        history: &[HistoryPoint],
        low: &BigUint,
        low_price: f64,
        ...
        // f(x) = price(x) - target
        // f1 = low_price - target, f2 = high_price - target
        let f1 = low_price - target;
        let f2 = high_price - target;

        // We need a third point at the midpoint
        // If we have history, use the most recent point that's close to midpoint
        // Otherwise, fall back to geometric mean (bisection)

        // Find the most recent history point that could serve as our midpoint evaluation
        let mid_point = history
            .iter()
            .rev()
            .find(|p| p.amount_f64 > low_f64 && p.amount_f64 < high_f64);

        let (x3, f3) = if let Some(p) = mid_point {
            (p.amount_f64, p.price - target)
        } else {
            // No interior point available, fall back to bisection
            return fallback;
        };

        // Ridders' formula:
        // x4 = x3 + (x3 - x1) * sign(f1 - f2) * f3 / sqrt(f3^2 - f1*f2)
        // where x1 = low, x2 = high

        // The discriminant under the sqrt
        let discriminant = f3 * f3 - f1 * f2;

        if discriminant < 0.0 {
            // This shouldn't happen if f1 and f2 have opposite signs
            // and f3 is between them, but handle gracefully
            return fallback;
        }

        let sqrt_disc = discriminant.sqrt();
        if sqrt_disc < MIN_DIVISOR {
            return fallback;
        }

        let sign = if f1 > f2 { 1.0 } else { -1.0 };

        let x4 = x3 + (x3 - low_f64) * sign * f3 / sqrt_disc;

        // Ensure result is within bounds
        if x4.is_finite() && x4 > low_f64 && x4 < high_f64 {
            if let Some(amount) = BigUint::from_f64(x4) {
                if let Some(safe) = safe_next_amount(amount, low, high) {
                    return safe;
                }
            }
        ...
        // If Ridders' estimate is out of bounds, try linear interpolation
        let dp = high_price - low_price;
        if dp.abs() > MIN_DIVISOR {
            let x_linear = low_f64 + (target - low_price) * (high_f64 - low_f64) / dp;
            if x_linear.is_finite() && x_linear > low_f64 && x_linear < high_f64 {
                if let Some(amount) = BigUint::from_f64(x_linear) {
                    if let Some(safe) = safe_next_amount(amount, low, high) {
                        return safe;
                    }
                }
...
impl ProtocolSimExt for RiddersStrategy {
    fn swap_to_price(
        &self,
        state: &dyn ProtocolSim,
        target_price: f64,
