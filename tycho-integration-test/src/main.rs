mod protocol_stream_processor;
mod rfq_stream_processor;
...
use crate::{
    ...
    rfq_stream_processor::RfqStreamProcessor,
...
async fn main() -> miette::Result<()> {
    let _hook_result = miette::set_hook(Box::new(|_| Box::new(NarratableReportHandler::new())));
    dotenv().ok();
...
async fn process_update_state(
    ...
    if tokens_len < 2 {
        warn!("Component has less than 2 tokens, skipping...");
        return;
    }
